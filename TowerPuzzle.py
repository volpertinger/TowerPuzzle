...
class TowerPuzzle:
    ...
    def count_visibility_right(self, index):
        result = 0
        max_height = 0
        length = len(self.field[0])
        for i in range(length):
            if max_height < self.field[index][length - i - 1]:
                max_height = self.field[index][length - i - 1]
                result += 1
        return result
    ...
    def count_visibility_down(self, index):
        ...
        length = len(self.field[0])
        for i in range(length):
            if max_height < self.field[length - i - 1][index]:
                max_height = self.field[length - i - 1][index]
                result += 1
        return result
    ...
    def is_solved(self):
        ...
        for i in range(len(self.visibility_down)):
